...
int inference_clip_model(rknn_app_context_t* app_ctx, image_buffer_t* img,
  ...
  float img_output[app_ctx->img.output_attrs[0].dims[0] *
                   app_ctx->img.output_attrs[0].dims[1]];
  float text_output[text_num * app_ctx->text.output_attrs[0].dims[1]];
  if (app_ctx->img.output_attrs[0].dims[1] !=
      app_ctx->text.output_attrs[0].dims[1]) {
    printf(
        "The dimensions of the img and text model output are not the same! "
        "Please confirm that are consistent");
    exit(-1);
  }
  ...
  for (int i = 0; i < text_num; i++) {
    std::vector<int> token =
        app_ctx->clip_tokenize->tokenize(input_texts[i], sequence_len, true);
  ...
  for (int i = 0; i < text_num; i++) {
    ret = inference_clip_text_model_utils(
        &(app_ctx->text), tokens + (i * sequence_len),
        text_output + (i * app_ctx->text.output_attrs[0].dims[1]));
    if (ret != 0) {
...
int inference_clip_image_only(rknn_app_context_t* app_ctx, image_buffer_t* img,
  ...
  if (app_ctx == NULL || img == NULL || img_output_ptr == NULL ||
      feature_dim == NULL) {
    printf("error: inference_clip_image_only invalid params\n");
    return -1;
  }
  ...
  *feature_dim = feature_dim_value;
  ...
  if (img_output == NULL) {
    printf("error: malloc img_output failed\n");
    return -1;
  }
  memset(img_output, 0, feature_dim_value * sizeof(float));

  printf("info: start clip image inference...\n");
  ret = inference_clip_image_model_utils(&app_ctx->img, img, img_output);
  if (ret != 0) {
    printf("error: clip image inference failed ret=%d\n", ret);
    free(img_output);
    return -1;
  }
